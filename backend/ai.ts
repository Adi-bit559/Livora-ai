import { RoommateProfile, Property, Review, User } from '@prisma/client';
 ...
 */
export function calculatePlatformFee(monthlyRent: number): number {
  return Math.round(monthlyRent * 0.02);
}

export function calculateRoommateMatch(
  profile1: Partial<RoommateProfile>,
  profile2: Partial<RoommateProfile>
): MatchResult {
  // 1. Sleep Schedule (15%)
  let sleepScore = 70;
  if (profile1.sleepSchedule === profile2.sleepSchedule) {
    ...
    (profile1.sleepSchedule === 'EARLY_BIRD' && profile2.sleepSchedule === 'NIGHT_OWL') ||
  ...
  if (profile1.cleanlinessLevel === profile2.cleanlinessLevel) {
    ...
    (profile1.cleanlinessLevel === 'HIGH' && profile2.cleanlinessLevel === 'RELAXED') ||
...
export function calculatePropertyTrustScore(
  property: Partial<Property> & { owner?: Partial<User>; reviews?: Partial<Review>[] }
): number {
  let score = 50;
  ...
  if (property.owner?.phoneVerified) score += 10;
  if (property.owner?.emailVerified) score += 10;
  ...
  if (property.reviews && property.reviews.length > 0) {
    const avgAccuracy =
      property.reviews.reduce((acc, r) => acc + (r.listingAccuracy || 5), 0) / property.reviews.length;
  ...
  if (property.powerBackup) score += 10;
  if (property.foodAvailable) score += 5;
  score += 5;

  return Math.min(99, Math.max(30, Math.round(score)));
}

export function predictPropertyVacancy(property: Partial<Property> & { occupancyRate?: number }): {
...
export function rankPropertyRecommendations(
  properties: any[],
  userProfile?: Partial<RoommateProfile>
): Array<{ property: any; matchScore: number; reasons: string[] }> {
